#!/usr/bin/env bash
#
# Job "prepare_services".
# Execute the prepare-service target of the makefile due to execute commands from php containers of a service.
#
# Parameters:
#   --service
#   --project-name
...
#
write_info "# JOB: Prepare services"
...
for option in "$@"; do
    case ${option} in
        ...
        ;;
        --job-init-service=*)
            export PHP_INIT_SERVICES="${option#*=}"
            shift
        ;;
        *) shift ;;
    esac
...
# we run init entrypoint of all php services
run_cmd "make -s prepare-services"
